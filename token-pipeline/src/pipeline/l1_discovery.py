...
class L1Discovery(PipelineLayer):
    ...
    def run(self, input_data: List[Any] = None) -> List[Dict[str, Any]]:
        ...
        for token_data in discovered_tokens:
            existing = self.repository.get_token(
                token_data["chain"], token_data["contract_address"]
            )
            if not existing:
                # Save to DB
                ...
                token_data["pool_age_minutes"] = (
                    datetime.now(timezone.utc) - token_data["pool_created_at"]
                ).total_seconds() / 60
        ...
        logger.info(
            f"--- L1 Discovery End (New: {len(new_tokens)}, Total Found: {len(discovered_tokens)}) ---"
        )
        return new_tokens

    def _map_gecko_pool_to_token(
        self, chain: str, item: Dict[str, Any]
    ) -> Dict[str, Any]:
        try:
            attrs = item["attributes"]
            ...
            return {
                "chain": chain,
                "contract_address": attrs[
                    "address"
                ],  # WARNING: This is POOL address, not Token address.
