...
def generate_report(repository: TokenRepository):
    ...
    from src.db.models import Token, ScanResult
    
    # repository.db is a Session instance
    total_tokens = repository.db.query(Token).count()
    dropped_tokens = repository.db.query(Token).filter(Token.status == "dropped").count()
    active_count = repository.db.query(Token).filter(Token.status == "active").count()
    watching_count = repository.db.query(Token).filter(Token.status == "watching").count()
    ...
    pass_l3_tokens = repository.db.query(Token).join(ScanResult).filter(
        Token.status == "active",
        ScanResult.layer == "L3",
    ...
    if pass_l3_tokens:
        ...
        for token in pass_l3_tokens:
            # Get latest L3 score
            l3_res = repository.db.query(ScanResult).filter(
                ScanResult.token_id == token.id,
                ScanResult.layer == "L3"
